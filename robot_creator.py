...
class Creator:
    def __init__(self, numRobots, numFoods):
        ...
        self.P = [robot.Robot(i,random.randint(0,800),random.randint(0,600), self.F, \
    ...
    def update(self):
        ...
        for f in self.F:
            ...
            if f.time > f.child_time and len(self.F) < 500:
        ...
        for p in self.P:
            ...
            if p.food_eaten >= p.food_to_reproduce:
                self.P.append(robot.Robot(p.id,p.rect.x,p.rect.y, \
                        self.F,velocity=p.velocity+(random.uniform(-1,1) if random.randint(0,10)< self.mutation_rate*10 else 0),\
